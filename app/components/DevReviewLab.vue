...
onMounted(() => {
  ...
  store.detail = {
    ...
    // Everything the markdown renderer has to survive, in the shape a real
    // description arrives in: the template's own HTML comments, headings, a
    // table, a task list, a nested list, a fence, a fold, a quote and mentions.
    body: [
      '<!-- Thanks for opening a request! Fill in the sections below and delete',
      '     anything that does not apply. This comment is not shown. -->',
      '## What this changes',
      '',
      ...
      '| | Before | After |',
      '| --- | --- | --- |',
      '| Requests to draw the page | 6 | 2 |',
      '| Header | an icon rail | three labelled lines |',
      '| Threads | a browser tab | here |',
      '',
      '### Still to do',
      '',
      '- [x] one status call',
      '- [x] settle a thread where it is read',
      '- [ ] the same for GitLab discussions',
      '',
      'The roll-up is one function now:',
      '',
      '```ts',
      'const standing = computed(() =>',
      "  checks.value.every((one) => one.state === 'success') ? 'green' : 'blocked'",
      ')',
      '```',
      '',
      '<details>',
      '<summary>The old shape, for comparison</summary>',
      '',
      'Four flags, three of which meant the same thing, and a `loading` that was',
      'true while any of them was.',
      '',
      '</details>',
      '',
      '> Read it on a narrow window before merging — the panel only shows up',
      '> beside the diff now.',
      '',
      'Thanks @nadia and @sam for the earlier pass. Closes #64, and unblocks #71.'
    ].join('\n'),
    state: 'open',
    draft: false,
