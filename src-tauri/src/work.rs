...
mod tests {
    ...
    /// A repository with one commit, for the tests that need a real one.
    fn repo() -> Fixture {
    ...
    fn renaming_a_stash_leaves_it_where_it_is_in_the_list() {
        ...
        for (name, line) in [("first", "a"), ("second", "b"), ("third", "c")] {
            repo.write("f.txt", &format!("one\n{line}\n"));
            stash_push(&repo.state, Some(name), false).expect("stash");
        }

        // The middle one: `git stash store` would have moved it to the top,
        // which is the whole reason the reflog is rewritten instead.
        stash_rename(&repo.state, 1, "renamed in place").expect("rename");

        let list = repo.stashes();
        assert!(list[0].contains("third"), "{list:?}");
        assert!(list[1].contains("renamed in place"), "{list:?}");
        assert!(list[2].contains("first"), "{list:?}");
        // Still on the branch it was made on, and still the same three.
        assert!(list[1].contains("On main"), "{list:?}");
    ...
    fn a_renamed_stash_still_holds_what_it_held() {
        let repo = repo();
        repo.write("f.txt", "one\nkept\n");
        stash_push(&repo.state, Some("before"), false).expect("stash");

        stash_rename(&repo.state, 0, "after").expect("rename");
    ...
    #[test]
    fn a_stash_needs_a_name_to_be_renamed_to() {
        let repo = repo();
        repo.write("f.txt", "one\ntwo\n");
        stash_push(&repo.state, Some("named"), false).expect("stash");

        assert!(stash_rename(&repo.state, 0, "   ").is_err());
        assert!(repo.stashes()[0].contains("named"));
    }

    #[test]
    fn deleting_untracked_files_leaves_the_tracked_ones_alone() {
        let repo = repo();
        repo.write("new.txt", "never seen\n");
        repo.write("f.txt", "one\nchanged\n");

        delete_untracked(&repo.state, &["new.txt".to_string()]).expect("delete");

        assert_eq!(repo.read("new.txt"), "");
        // The tracked file is untouched, changes and all: `git clean` will not
        // touch it, which is the guard that makes this safe to offer.
        assert_eq!(repo.read("f.txt"), "one\nchanged\n");
    }

    #[test]
    fn a_tracked_file_survives_being_handed_to_the_delete() {
        let repo = repo();
        repo.write("f.txt", "one\nchanged\n");

        let _ = delete_untracked(&repo.state, &["f.txt".to_string()]);
